...
Les tests sont écrits en Python et exécutés avec l’outil ``pytest``.
...
Un test unitaire a été créé pour vérifier que les entrées (ex: coordonnées, paramètres)
...
Il s’agit d’un test d’intégration simple mais pertinent.
